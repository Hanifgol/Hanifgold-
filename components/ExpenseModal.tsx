    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" aria-modal="true" role="dialog">
      <div className="bg-white rounded-xl shadow-xl max-w-lg w-full">
        <div className="p-6 border-b border-medium-gray">
          <h2 className="text-xl font-bold text-secondary">{expenseToEdit ? 'Edit Expense' : 'Add New Expense'}</h2>
          <p className="text-sm text-gray-500">Log a cost for your business.</p>
        </div>
        <div className="p-6 space-y-4">
          <div>
            <label htmlFor="description" className="block text-sm font-bold text-dark-gray">Description*</label>
            <input
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label htmlFor="amount" className="block text-sm font-bold text-dark-gray">Amount (NGN)*</label>
              <input
            ...
            <div>
              <label htmlFor="date" className="block text-sm font-bold text-dark-gray">Date*</label>
              <input
          ...
          <div>
            <label htmlFor="category" className="block text-sm font-bold text-dark-gray">Category*</label>
            <select
          ...
          <div>
            <label htmlFor="quotationId" className="block text-sm font-bold text-dark-gray">Link to Project (Optional)</label>
            <select
        ...
        <div className="p-4 bg-light-gray border-t border-medium-gray flex justify-end gap-4">
          ...
          <button type="button" onClick={handleSave} disabled={!isFormValid} className="px-6 py-2 bg-primary text-white font-bold rounded-lg hover:bg-sky-600 disabled:bg-gray-400">Save Expense</button>
        </div>
      </div>
    </div>
  );
